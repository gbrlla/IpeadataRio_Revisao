...
### Gráfico 1: Quantidade de acessos no Ipeadata.

```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7.5, fig.height = 3.75}
nomes <- list.files('QtdAcessIpeadata')
aces <- as_tibble(NULL)

for (i in 1:length(nomes)) {
  aces_aux <- as_tibble(data.frame(A = paste0(substr(nomes[i], 18, 24), "-01"),
                            B = sum(read.csv2(paste0("QtdAcessIpeadata/",nomes[i]))[ , 8])))

  aces <- bind_rows(aces , aces_aux)
}

library(dygraphs)

acests <- ts(data = aces$B/1000, start = c(2000, 3), frequency = 12)

dygraph(acests, ylab = "Quantidade de Acessos (x 1000)") %>%
   dySeries("V1", label = "Acessos") %>%
  dyBarChart() %>%
...
atrasos <- RPostgreSQL::dbGetQuery(con, 
                                      paste0("SELECT TO_CHAR(data_atraso,  'YYYY-MM-DD') data0, ",
                                             "atraso_macro FROM public.tb_ipeadata_atrasos;"))
